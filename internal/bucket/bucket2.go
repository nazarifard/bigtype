...
func (b *Bucket2) Write(index int, data []byte) (offset uint16) {
	...
	b.FreeCell.Body = &b.cellar[freeOffset+8] // : freeOffset+8-10+freeLen]
...
func (b *Bucket2) Read(offset uint16) (data []byte) {
	...
	if ok && cell.Head.Len == cell.Tail.Tlen && cell.Head.Status {
		return unsafe.Slice(cell.Body, cell.Head.Len-8-2)
	} else {
...
func (b *Bucket2) Defrag(addr map[int64]uint16) {
	...
	b.FreeCell.Head = (*Head)(unsafe.Pointer(&b.cellar[1+len(dst)]))
	b.FreeCell.Head.Len = freeLen
	b.FreeCell.Head.Status = Dead
	b.FreeCell.Head.SetIndex(FreeCellIndex)
	b.FreeCell.Body = (*byte)(unsafe.Pointer(&b.cellar[1+len(dst)+8]))
